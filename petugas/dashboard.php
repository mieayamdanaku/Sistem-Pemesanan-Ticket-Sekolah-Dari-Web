...
// --- 2. LOGIKA STATISTIK ANGKA (FINANSIAL PENDAPATAN) ---
...
<head>
    ...
    <style>
        ...
        /* Style untuk tombol pendapatan (finance) */
        ...
        .card-finance:hover { transform: translateY(-5px); box-shadow: 0 15px 30px rgba(0,0,0,0.1); color: white; }
        .bg-blue { background: linear-gradient(135deg, #2563EB, #1D4ED8); }
        .bg-green { background: linear-gradient(135deg, #10B981, #059669); }
        .bg-orange { background: linear-gradient(135deg, #F59E0B, #D97706); }
        .bg-purple { background: linear-gradient(135deg, #8B5CF6, #7C3AED); }
        
        /* Style untuk card operasional mini dan chart */
...
<body>
    ...
    <div class="container mt-5 mb-5">
        ...
        <div class="mb-4">
            ...
            <p class="text-muted">Pantau pendapatan tiket, kelola event, dan monitoring tiket masuk.</p>
        </div>

        <!-- 1. TOMBOL RINGKASAN PENDAPATAN & FILTER GRAFIK -->
        <h5 class="fw-bold mb-3">Ringkasan Pendapatan & Filter Grafik</h5>
        <div class="row mb-4">
            <div class="col-md-3 mb-3">
                <a href="?view=semua" class="card card-finance bg-blue p-4 <?= ($view == 'semua') ? 'active-filter' : '' ?>">
                    <div class="small fw-bold opacity-75 text-uppercase">Total Pendapatan</div>
                    <h3 class="fw-800 m-0 mt-1">Rp <?= number_format($omzet_total) ?></h3>
                </a>
            </div>
            <div class="col-md-3 mb-3">
                <a href="?view=hari" class="card card-finance bg-green p-4 <?= ($view == 'hari') ? 'active-filter' : '' ?>">
                    <div class="small fw-bold opacity-75 text-uppercase">Hari Ini</div>
                    <h3 class="fw-800 m-0 mt-1">Rp <?= number_format($omzet_hari_ini) ?></h3>
                </a>
            </div>
            <div class="col-md-3 mb-3">
                <a href="?view=minggu" class="card card-finance bg-orange p-4 <?= ($view == 'minggu') ? 'active-filter' : '' ?>">
                    <div class="small fw-bold opacity-75 text-uppercase">Minggu Ini</div>
                    <h3 class="fw-800 m-0 mt-1">Rp <?= number_format($omzet_minggu_ini) ?></h3>
                </a>
            </div>
            <div class="col-md-3 mb-3">
                <a href="?view=bulan" class="card card-finance bg-purple p-4 <?= ($view == 'bulan') ? 'active-filter' : '' ?>">
                    <div class="small fw-bold opacity-75 text-uppercase">Bulan Ini</div>
                    <h3 class="fw-800 m-0 mt-1">Rp <?= number_format($omzet_bulan_ini) ?></h3>
                </a>
            </div>
        </div>

        <!-- 2. GRAFIK PENJUALAN -->
        <div class="card chart-card p-4 mb-5 shadow-sm">
            <h5 class="fw-bold mb-4">📈 <?= $judul_grafik ?></h5>
            <div style="height: 350px;"><canvas id="salesChart"></canvas></div>
        </div>

        <!-- 3. TOMBOL OPERASIONAL (Tampil jika view = semua, ditaruh di bawah grafik) -->
        <?php if($view == 'semua'): ?>
        <h5 class="fw-bold mb-3">Statistik Operasional</h5>
        <div class="row mb-5">
            <div class="col-md-3 mb-3">
                <a href="#riwayat-section" class="card mini-card p-3 d-flex flex-row align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <h4 class="fw-800 m-0 text-dark"><?= number_format($total_tiket_terjual) ?></h4>
                    </div>
                </a>
            </div>
            <div class="col-md-3 mb-3">
                <a href="kelola_event.php#aktif-section" class="card mini-card p-3 d-flex flex-row align-items-center gap-3">
                    ...
                    <div>
                        <div class="text-muted small fw-bold text-uppercase">Event Aktif</div>
                        <h4 class="fw-800 m-0 text-dark"><?= $event_aktif ?></h4>
                    </div>
                </a>
            </div>
            <div class="col-md-3 mb-3">
                <a href="kelola_event.php#expired-section" class="card mini-card p-3 d-flex flex-row align-items-center gap-3">
                    ...
                    <div>
                        <div class="text-muted small fw-bold text-uppercase">Event Selesai</div>
                        <h4 class="fw-800 m-0 text-dark"><?= $event_selesai ?></h4>
                    </div>
                </a>
            </div>
            <div class="col-md-3 mb-3">
                <a href="kelola_event.php" class="card mini-card p-3 d-flex flex-row align-items-center gap-3">
                    ...
                    <div>
                        <div class="text-muted small fw-bold text-uppercase">Total Event</div>
                        <h4 class="fw-800 m-0 text-dark"><?= $total_event ?></h4>
                    </div>
                </a>
            </div>
        </div>
